...
| Lesson Player (separate web app, link-out only) | Active | Opens a lesson in the player UI from CMS lesson edit page (`/lab/lesson/:lessonId`). | `LESSON_PLAYER_BASE_URL` (server), `NEXT_PUBLIC_LESSON_PLAYER_BASE_URL` (fallback), hardcoded fallback `http://localhost:3001` | Lesson Player Team | `app/edit-lesson/[lessonId]/page.tsx` |
